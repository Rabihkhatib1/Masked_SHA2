...
/* Compress */
    ...
    // Precomputing message scheduler   
   ...
   for (i = 0; i < 8; i++) {
        ...
        md->shares[i].xr = (md->shares[i].xr + S[i].xr) & MODULO;
        md->state[i] = arith_comb(md->shares[i]);
    }
...
void print_share(const share* s) {
    printf("share:\n");
    printf("  xs = 0x%016lx\n", s->xs);
    printf("  xr = 0x%016lx\n", s->xr);
}

void print_share_intlv(const share_intlv* s) {
    printf("share_intlv:\n");
    printf("  x0 = 0x%08x\n", s->x0);
    printf("  x1 = 0x%08x\n", s->x1);
    printf("  x2 = 0x%08x\n", s->x2);
...
int main() {
    ...
    share test = {.xs = 0x48656c6c6f2c2077, .xr = 0xf08a05e2d0065faf};
    share_intlv test_intlv = {.x0 = 0x5faf2077, .x1 = 0xd0066f2c, .x2 = 0x05e26c6c, .x3 = 0xf08a4865};

    b2a(&test);
    b2a_intlv(&test_intlv);
    // printf("",b2a(&test));
    print_share(&test);
